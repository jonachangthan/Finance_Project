...
if not os.path.exists(f"../data/strategy/3/"):
    os.makedirs(f"../data/strategy/3/")
output: list[dict] = []
...
for year in range(1998, 2009):
    ...
    # 預測
    grid.fit(X_train, Y_train)  # 預測測試集
    ...
    if len(output) != 0:
        ...
        # 計算年報酬率
    output_df.to_csv(f"../data/strategy/3/SplitBy{year}.csv", index=False)
    if "data.json" not in os.listdir("../data/strategy/3/"):
        with open(f"../data/strategy/3/data.json", "w", encoding="utf-8") as outfile:
            json.dump(
                {},
                outfile,
                ensure_ascii=False,
            )
    with open(f"../data/strategy/3/data.json", "r", encoding="utf-8") as outfile:
